...
{
	uint64_t result;
	rtm::memCopy(&result, sizeof(result), p, sizeof(result));
	return result;
}
...
template <typename T>
static inline void SwapValues(T& f, T& s)
{
	T t = f;
	f = s;
	s = t;
...
typedef struct uint128_t
{
	...
	inline uint128_t(uint64_t f, uint64_t s) : first(f), second(s) {}
...
{
	return HashLen16(hashCity64(s, _len) - seed0, seed1);
}

uint64_t hashCity64(const void* _key, uint32_t _len, uint64_t seed)
{
	const char* s = (const char*)_key;
