...
namespace rtm {
	...
	void sortRadix(uint32_t* _keys, uint32_t* _tempKeys, Ty* _values, Ty* _tempValues, uint32_t _size)
	{
		uint16_t histogram[RTM_RADIXSORT_HISTOGRAM_SIZE];
		uint16_t shift = 0;
		for (uint32_t pass=0; pass<3; ++pass)
	...
	void radixSort64(uint64_t* _keys, uint64_t* _tempKeys, uint32_t _size)
	{
		uint16_t histogram[RTM_RADIXSORT_HISTOGRAM_SIZE];
		uint16_t shift = 0;
